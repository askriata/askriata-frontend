...
export default async function MediaDetailPage({ params }) {
  ...
  const [seasonsResult, charactersResult, unitsResult] = await Promise.allSettled([
      ...
      {unitGroups.size > 0 && (
        <Section title={unitTypeLabel}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            {[...unitGroups.entries()].map(([groupName, groupUnits]) => (
              <div key={groupName}>
                {unitGroups.size > 1 && (
                  ...
                  {groupUnits
                    ...
                    .map((u) => (
                      <div key={u.id} style={styles.unitRow}>
                        ...
                        <span style={{ fontSize: 14 }}>{u.title || `${unitTypeLabel.slice(0, -1)} ${u.number}`}</span>
                      </div>
                    ))}
                </div>
              </div>
