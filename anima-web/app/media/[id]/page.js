...
import AddMediaStaffForm from '../../../components/AddMediaStaffForm';
...
export default async function MediaDetailPage({ params }) {
  ...
  const [seasonsResult, charactersResult, unitsResult, arcsResult, staffResult] = await Promise.allSettled([
    ...
    <article style={{ maxWidth: 720 }}>
      ...
      <Section title="Characters">
        {characters.length > 0 ? (
          ...
          <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 14 }}>No characters added yet.</p>
        )}
      ...
      <Section title={unitTypeLabel}>
        {units.length > 0 ? (
          ...
          <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 14 }}>
            No {unitTypeLabel.toLowerCase()} added yet.
          </p>
        )}
      ...
      <Section title="Staff">
        {staff.length > 0 ? (
          <div style={{ ...styles.characterGrid, marginBottom: 14 }}>
            {staff.map((s) => (
              <div key={`${s.staff_id}-${s.role}`} style={styles.characterCard}>
                <div className="eyebrow">{s.role}</div>
                <div style={{ fontSize: 14, marginTop: 4 }}>{s.staff?.name}</div>
              </div>
            ))}
          </div>
        ) : (
          <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 14 }}>No staff credited yet.</p>
        )}
        <AddMediaStaffForm mediaId={params.id} />
      </Section>
    </article>
  );
}
